...
initialGuess = [800, 1.115, 0.0005, 100, 1.115, 0.003]
nBins = 80
...
tree = file["fit4c"]
# navigate to branch named "mLambda"
branch = tree["mLambda"]
# get data
data = branch.array()
data = np.array(data)
...
plt.legend()
plt.xlabel('m(mLambda) [GeV/c$^2$]')
plt.ylabel('counts')
plt.title('Double Gaussian fit to m(mLambda) Inclusive MC data')
#plt.show()
plt.savefig('C:/Users/benja/OneDrive/Documents/15c project/USTC/plot-code/imgs/double-gaussian-inclusiveMC_lambda.png')
