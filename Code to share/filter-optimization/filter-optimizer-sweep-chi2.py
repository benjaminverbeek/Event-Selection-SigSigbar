...
cut_max = 200 # (already cut 200 in ES.cxx)
cut_min = 1 
...
for chi2_cut in sweep_range:
    ...
    FOM = S/np.sqrt(S+weight*B)
    # FOM = S/np.sqrt(S+B) + 200*S/(B+S)
...
plt.legend()
print(f"Max FOM: {max_FOM:.4f} at {sweep_range[max_FOM_index]:.4f}")
print(f"First FOM > 0.98 max : {FOM98:.4f} at {sweep_range[index98]:.4f}")

nme = "$w_{\Sigma^0}$"
plt.title("FOM vs $c_\chi$", fontsize=12)
plt.figtext(0.5, 0.35, f'$w_\Lambda$ = {lambda_window:.4f} GeV, {nme} = {sigma_window:.4f}', fontsize=10)
plt.savefig(r"C:\Users\benja\OneDrive\Documents\15c project\USTC\all_ana_roots"+f"/filter-optimization/plots/FOM_vs_cChi2", dpi=300)
plt.show()
