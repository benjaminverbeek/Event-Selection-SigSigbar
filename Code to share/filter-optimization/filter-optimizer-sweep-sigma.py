...
cut_max = 0.05 # (already cut 200 in ES.cxx)
cut_min = 0.001 
# get a linspace of chi2 values to sweep over
sweep_range = np.linspace(cut_min, cut_max, 100)
...
FOMs = []   # list of FOMs
for sigma_window in sweep_range:
...
#         Signal: {S} 
#         FOM: {FOM:.4f} +/- {dFdS:.4f}
# Window size: {sigma_window:.4f} GeV
#     {"-"*30}""")
#     print(f"Efficiency: {S/23032*100:.4f} % \t (relative to 23032 signal events, no filtering after ES)")
...
plt.xlabel("sigma window [GeV]", fontsize = 12)
plt.ylabel("FOM", fontsize = 12)
plt.grid()
# find intersection of this line with FOMs
# find the first index of FOMs that is greater than 0.98*max_FOM
...
plt.scatter(sweep_range[index98], FOM98, marker="+", color="green", label=f"FOM>0.98 max : {FOM98:.4f} at {sweep_range[index98]:.4f} GeV")

# plot a horizontal red dashed line at max FOM - 5% max
plt.axhline(0.98*max(FOMs), color="red", linestyle="--", label=f"98% of max FOM: {max(FOMs)*0.98:.4f}", linewidth=0.8)

# and mark the highest FOM
max_FOM = max(FOMs)
max_FOM_index = FOMs.index(max_FOM)
plt.scatter(sweep_range[max_FOM_index], max_FOM, marker="x", color="red", label=f"Max FOM: {max_FOM:.4f} at {sweep_range[max_FOM_index]:.4f} GeV")
plt.legend()
print(f"Max FOM: {max_FOM:.4f} at {sweep_range[max_FOM_index]:.4f} GeV")
print(f"First FOM > 0.98 max : {FOM98:.4f} at {sweep_range[index98]:.4f} GeV")

plt.title("FOM vs $w_{\Sigma^0}$", fontsize=12)
plt.figtext(0.5, 0.35, f'$w_\Lambda$ = {lambda_window:.4f} GeV, $c_\chi$ = {chi2_cut:.4f}', fontsize=10)
plt.savefig(r"C:\Users\benja\OneDrive\Documents\15c project\USTC\all_ana_roots"+f"/filter-optimization/plots/FOM_vs_wSigma", dpi=300)
plt.show()
print("Saved figure.")
